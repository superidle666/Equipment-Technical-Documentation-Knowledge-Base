...
└───────────────┬──────────────┘  │ Milvus：向量索引              │
                ...
                └─────────────────┴─────────────────────────────┘
2.1 文档导入链路
管理端上传 PDF 或 Markdown 文件。
后端创建文档记录和导入任务。
导入工作流解析文件，必要时执行 PDF 转 Markdown。
提取并规范化 Markdown 图片地址，图片写入 MinIO。
按标题、段落、表格等内容切分知识块。
按知识库配置执行实体识别。
生成向量并写入 Milvus。
将切片、来源位置和元数据持久化到 MySQL。
管理端通过导入任务接口查看处理进度和错误信息。
2.2 用户问答链路
用户端创建或切换会话并提交问题。
后端校验用户、会话和知识库状态。
查询工作流进行问题准备、向量召回、可选 HyDE、可选联网搜索、RRF 合并和重排序。
从 MySQL 补齐文档、切片、页码、标题和图片等来源信息。
生成带来源上下文的回答。
通过普通响应或 SSE 流式返回答案、来源和关联图片。
会话与消息历史写入 MongoDB。
3. 项目技术栈
3.1 前端
Vue 3 + <script setup>
...
管理端仪表盘统计和最近活动展示。
9. 后续规划
完善会话生命周期管理，包括归档、批量操作、搜索、状态和更细粒度的历史管理。
实现知识库权限管理，支持为指定用户或角色授权，并在知识库列表、会话创建、历史读取和检索接口统一校验。
完善权限审计，记录授权、撤销、越权拦截和管理操作。
优化图片与回答内容的关联，避免同批召回中的无关图片被带入答案。
增强检索质量评估、召回过滤、实体识别和回答引用准确性。
逐步补充自动化测试、部署脚本和生产环境监控。
10. 开发约束
